...
6. Railway production jobs are configured in UTC: `analytics-courier-sync` every 30 minutes, `analytics-ga4-snapshot` daily at 02:15, `analytics-meta-snapshot` daily at 02:30, and `analytics-ga4-events` every 10 minutes. Add the Google snapshot only after Google grants Explorer or higher API access. The event job validates each queued payload before collection and never creates events from historical backfill.
