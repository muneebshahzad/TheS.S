...
| GA4 `491963638` | Analytics Data API is enabled and the service account has Viewer access. Live reporting validation succeeded. Web stream `G-0798KTT0NT` is configured in Railway and the live storefront. The Measurement Protocol secret is stored only in Railway and its debug-endpoint validation completed with zero messages. |
...
6. Railway production jobs are configured in UTC: `analytics-courier-sync` every 30 minutes, `analytics-ga4-snapshot` hourly at minute 20, `analytics-meta-snapshot` hourly at minute 25, and `analytics-ga4-events` every 10 minutes. Snapshot commands derive the current reporting date in `ANALYTICS_TIMEZONE`, not the worker's UTC date, and refresh the trailing seven days to absorb platform processing updates. Add the Google snapshot only after Google grants Explorer or higher API access. The event job validates each queued payload before collection and never creates events from historical backfill.
...
Transport outcomes are deliberately explicit: pending → attempting → sent / invalid / uncertain / expired. A crash after claiming or a collection timeout needs manual reconciliation. Measurement Protocol has no universal exactly-once acknowledgement; automatically retrying uncertain sends could duplicate events. Inspect a specific event against GA4 before any manual retry; do not bulk reset uncertain/attempting rows. The production credentials and a schema-correct synthetic payload passed Google's debug endpoint with zero validation messages; inspect DebugView after the next genuine, consented final transition.
...
Production activation, signed-webhook replay protection, full ledger uniqueness, GA4/Meta reporting responses, Measurement Protocol credential validation and representative Shopify/courier samples have been verified. Remaining acceptance work is to inspect DebugView after the next genuine consented final transition and complete real-browser storefront consent/checkout coverage. Google Ads reporting starts after Google grants Explorer or higher API access.
